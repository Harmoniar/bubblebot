...
def compare_image_with_hash(image_file1: str, image_file2: str, max_dif: int = 1.5) -> bool:
    """
    说明:
        比较两张图片的hash值是否相同
    参数:
...
class BuildImage:
    ...
    async def apoint(self, pos: Tuple[int, int], fill: Optional[Tuple[int, int, int]] = None):
        """
        说明:
            异步 绘制多个或单独的像素
        参数:
    ...
    async def areplace_color_tran(
        self,
        src_color: Union[Tuple[int, int, int], Tuple[Tuple[int, int, int], Tuple[int, int, int]]],
        ...
        """
        self.loop.run_in_executor(None, self.replace_color_tran, src_color, replace_color)
...
class BuildMat:
    ...
    def __init__(
        ...
        *,
        ...
        if not y_index:
            ...
            _max_value = ceil(_max_value / eval("1" + "0" * (len(str(_max_value)) - 1))) * eval("1" + "0" * (len(str(_max_value)) - 1))
    ...
    def set_background(self, background: Optional[List[str]], type_: Optional[str] = None):
        """
        说明:
            设置背景图片
        参数:
    ...
    def _init_graph(
        ...
        A = BuildImage(self.w, self.h, font_size=font_size, font=self.font, background=background)
        if background:
        ...
        for s in new_text.split("\n"):
            ...
            for _ in range(s.count("[placeholder_")):
                ...
                if "font_size" in placeholder[2]:
                    ...
                    if r:
                        w, h = BuildImage(0, 0, font_size=int(r.group(1))).getsize(placeholder[3])
                        img_height = img_height if img_height > h else h
                        img_width += w
                else:
                    img_width += BuildImage(0, 0, font_size=font_size).getsize(placeholder[3])[0]
            ...
            # 开始画图
            A = BuildImage(img_width, img_height, color=color, font=font, font_size=font_size)
            ...
            for _ in range(s.count("[placeholder_")):
                if not s.startswith(f"[placeholder_{current_placeholder_index}]"):
                    ...
                    await A.atext((current_width, A.h - basic_font_h - 1), slice_[0], font_color)
                ...
                _img_h = int(A.h / 2 - text_img.h / 2) if new_text == "[placeholder_0]" else A.h - text_img.h
                await A.apaste(text_img, (current_width, _img_h - 1), True)
                current_width += text_img.w
                s = s[s.index(f"[placeholder_{current_placeholder_index}]") + len(f"[placeholder_{current_placeholder_index}]") :]
